...
export default function PreFlightAudit() {
  ...
  const auditQuestions = analysisResult?.auditQuestions;
  ...
  if (!analysisResult || !auditQuestions) return null;
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="space-y-3">
          {question.options.map((option, i) => {
            ...
            let optionClass =
              "border-border text-white hover:border-muted hover:bg-white/[0.02]";
            if (answered) {
              if (isCorrect) {
                ...
                optionClass = "border-border/50 text-muted/50";
              }
            }
            ...
            return (
              <button
                ...
                disabled={answered}
                className={`w-full text-left border rounded-lg p-4 font-mono text-sm transition-all flex items-start gap-3 ${optionClass} ${
                  answered ? "cursor-default" : "cursor-pointer"
                }`}
              >
                <span
                  className={`w-6 h-6 rounded-full border flex items-center justify-center text-xs shrink-0 mt-0.5 ${
                    answered && isCorrect
        ...
        {answered && (
          <div className="mt-4 p-4 bg-surface border border-border rounded-lg">
            <div className="font-mono text-[10px] tracking-[0.15em] text-muted mb-2">
              EXPLANATION
            </div>
