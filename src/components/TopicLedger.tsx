...
export default function TopicLedger() {
    ...
    return (
      <div className="flex-1 flex flex-col">
        ...
        <div className="flex-1 flex items-center justify-center">
          <p className="font-mono text-muted text-sm">No topics analyzed yet.</p>
        </div>
      </div>
    );
  }

  const { topics, flashcards, auditScores } = currentExam;
  const avgComplexity =
  ...
  const handleJump = (page: number) => {
    const pdfFile = currentExam.uploadedFiles.find(
      (f) => f.type === "application/pdf" && f.blobUrl
    );
    if (pdfFile) window.open(`${pdfFile.blobUrl}#page=${page}`, "_blank");
  };
    ...
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <div className="flex-1 p-8 overflow-auto">
        <h2 className="font-heading text-white text-2xl font-bold mb-1">
          {currentExam.subjectName} — {currentExam.examType} / Topic Ledger
        </h2>
              ...
              <div key={i}>
                <div className="grid grid-cols-[28px_1fr_200px_120px_100px_80px] gap-4 px-5 py-4 border-b border-border/50 hover:bg-white/[0.02] transition-colors items-center">
                  ...
                  <div className="font-mono text-xs">
                    {yieldVal !== null ? (
                      <span className={yieldVal >= 70 ? "text-accent" : yieldVal >= 40 ? "text-amber" : "text-danger"}>
                        {yieldVal.toFixed(0)}%
                      </span>
                    ) : (
                  ...
                  <div className="bg-bg/50">
                    {topic.subtopics.map((sub, j) => (
                      ...
                      >
                        ...
                        <div className="font-mono text-xs text-white/80">{sub.name}</div>
                        <span
                        ...
                        <div className="font-mono text-xs text-muted line-clamp-1">{sub.description}</div>
                        <button
